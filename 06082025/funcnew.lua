...
local function ragedash()
    pcall(function() if not canDash or not RootPart or Humanoid.Health <= 0 then return end end)
    canDash = false
    playRageDashSound()
    local Camera = workspace.CurrentCamera
...
local function kickself()
    ...
    local launchVector = (LookVector * _G.Config.kick_fw + Vector3.new(0, _G.Config.kick_up, 0))
