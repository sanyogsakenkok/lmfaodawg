		...
		for player, data in pairs(ESPPlayerCache) do
			...
			if _G.Config.skeleton and data.Skeleton then
				...
				if char then
					...
					drawLine(skeleton.Head, hrp, head)
					drawLine(skeleton.Torso, ll, rl)
					drawLine(skeleton.LeftLeg, hrp, ll)
					drawLine(skeleton.RightLeg, hrp, rl)
					drawLine(skeleton.LeftArm, hrp, la)
					drawLine(skeleton.RightArm, hrp, ra)
				end
			elseif data.Skeleton then
				-- Прячем линии, если skeleton отключён
				for _, line in pairs(data.Skeleton) do
					line.Visible = false
				end
			end
