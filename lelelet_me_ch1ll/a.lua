...
function AddESPToPlayer(player)
	...
	if player.Team.Name == "Survivor" and not _G.Config.survEnabled then return end
	if player.Team.Name == "Killer" and not _G.Config.killerEnabled then return end
		...
		elseif objectName == "Window" and obj.Name:match("Window%d") then
			...
			local function updateText()
				if not windBlocked.Value and not Blocked.Value then
                    ...
                    task.wait(1)
                    if _G.Config.unblock_window then
                        Blocked.Value = false
            ...
            if typeof(item) == "Instance" and item:IsA("BillboardGui") then
                ...
                if adornee and adornee:IsA("Instance") then
                    ...
                    if not isAlwaysVisible then
                        fadeAlpha = math.clamp(1 - (dist / _G.Config.ObjectsDistance), 0, 1)
                    end
                    ...
                    item.Enabled = isAlwaysVisible or dist <= _G.Config.ObjectsDistance
                end
            end
        end
    end)
