...
local function getSurvName()
    if input_TargetSurvName then
        local targetChar = workspace:FindFirstChild(input_TargetSurvName)
        if targetChar and targetChar:IsA("Model") then
            return input_TargetSurvName
        else
...
local function remotePlayers()
    ...
    local targetPlayer = Players:FindFirstChild(input_TargetSurvName)
    ...
    if isHooked then
        for _, hook in pairs(workspace:GetChildren()) do
            if hook:IsA("Model") and hook.Name:match("Hook%d") then
                ...
                if plr.Value == input_TargetSurvName then
...
local function playOverrideAnimation(animationId, play)
    if selected_Animation == "Snake It" then
        animationId = "10258272240"
    elseif selected_Animation == "Get Em" then
        animationId = "15919624781"
    elseif selected_Animation == "Think" then
        animationId = "121839807114786"
    elseif selected_Animation == "Bowshot" then
        animationId = "131833227843012"
    elseif selected_Animation == "1st place" then
        animationId = "118211011052686"
    elseif selected_Animation == "2nd place" then
        animationId = "133419229272999"
    elseif selected_Animation == "3rd place" then
        animationId = "83657343152240"
    elseif selected_Animation == "Point" then
        animationId = "16520476888"
    elseif selected_Animation == "Follow" then
        animationId = "16520472961"
    elseif selected_Animation == "Strange Movement" then
        animationId = "17006953862" 
    elseif selected_Animation == "Hooked" then
        animationId = "18906020019"
    elseif selected_Animation == "Picked Up" then
        animationId = "136738311253396"
    elseif selected_Animation == "Locker Grabbed" then
        animationId = "15947959258"
    elseif selected_Animation == "Escape" then
...
game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessed)
    ...
    elseif input.KeyCode == Enum.KeyCode.C then
        ...
        if closest then input_TargetSurvName = closest.Name end
    end
end)
print('b - done')
