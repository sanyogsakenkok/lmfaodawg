...
function AddESPToPlayer(player)
	...
	if player.Team.Name == "Survivor" and not _G.ESP.survivor then return end
	if player.Team.Name == "Killer" and not _G.ESP.killer then return end
...
function enableObjectESP(objectName, labelText, color)
		...
		elseif objectName == "Window" and obj.Name:match("Window%d") then
			...
			local function updateText()
				if not windBlocked.Value and not Blocked.Value then
                    ...
                    task.wait(1)
                    if _G.SExploits.unblock_window then
                        Blocked.Value = false
            ...
            if typeof(item) == "Instance" and item:IsA("BillboardGui") then
                ...
                if adornee and adornee:IsA("Instance") then
                    ...
                    if not isAlwaysVisible then
                        fadeAlpha = math.clamp(1 - (dist / _G.ESP.ObjectsDistance), 0, 1)
                    end
                    ...
                    if distanceLabel then
						if _G.ESP.enable_objectdistance then
                    ...
                    item.Enabled = isAlwaysVisible or dist <= _G.ESP.ObjectsDistance
                end
            end
        end
    end)
    ...
    game:GetService("Workspace").DescendantAdded:Connect(function(obj)
        if obj:IsA("Model") and obj.Name == "Trap" and _G.ESP.trap then
